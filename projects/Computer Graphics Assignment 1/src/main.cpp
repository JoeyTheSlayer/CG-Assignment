...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_AmbSpecShader", AmbSpecShader);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Assignment#1 Requirements"))
			{
				if (ImGui::Button("All lights On"))
				{
						...
						shader->SetUniform("u_AmbSpecShader", AmbSpecShader = 0);	
				}
				...
				if (ImGui::Button("Lights & Cell shading"))
				{
					shader->SetUniform("u_BothLights", BothLights = 0);
					shader->SetUniform("u_AmbLights", AmbLights = 0);
					shader->SetUniform("u_SpecLights", SpecLights = 0);
					shader->SetUniform("u_NoLights", NoLights = 0);
					shader->SetUniform("u_AmbSpecShader", AmbSpecShader = 1);
				}
		...
		Texture2D::sptr red = Texture2D::LoadFromFile("images/Car2-Red.png");
		Texture2D::sptr blue = Texture2D::LoadFromFile("images/Car2-Blue.png");
		Texture2D::sptr yellow = Texture2D::LoadFromFile("images/Car2-Yellow.png");
		Texture2D::sptr green = Texture2D::LoadFromFile("images/Car2-Green.png");
		Texture2D::sptr road = Texture2D::LoadFromFile("images/PossibleRoad.png");
		Texture2D::sptr butterfly = Texture2D::LoadFromFile("images/Butterfly.png");
		...
		//Start Creating Our OBJ's ////////////////////////////////////////////////////////////////////////////////////////

		GameObject obj2 = scene->CreateEntity("CarRed");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Car2.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj2.get<Transform>().SetLocalPosition(-1.0f, 0.0f, 1.0f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, 150.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("CarBlue");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Car2.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			obj3.get<Transform>().SetLocalPosition(3.0f, 0.0f, 1.0f);
			obj3.get<Transform>().SetLocalRotation(90.0f, 0.0f, 210.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj3);
		}
		
		GameObject obj4 = scene->CreateEntity("Road");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/PosibbleStraight.obj");
			obj4.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material3);
			obj4.get<Transform>().SetLocalPosition(0.0f, -30.0f, 0.3f);
		...
		GameObject obj5 = scene->CreateEntity("Butterfly");
		{
			// Build a mesh
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Butterfly.obj");
			
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material4);
			obj5.get<Transform>().SetLocalPosition(-2.0f, 0.0f, 5.0f);
			obj5.get<Transform>().SetLocalRotation(180.0f, -90.0f, 0.0f);
			...
			pathing->Points.push_back({ 4.0f, -4.0f, 3.0f });
			pathing->Points.push_back({ 4.0f,  4.0f, 4.0f });
			pathing->Points.push_back({ -4.0f,  4.0f, 3.0f });
			pathing->Speed = 2.0f;
		}

		GameObject obj6 = scene->CreateEntity("Butterfly2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Butterfly.obj");

			obj6.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material4);
			obj6.get<Transform>().SetLocalPosition(-2.0f, 0.0f, 5.0f);
			obj6.get<Transform>().SetLocalRotation(180.0f, -90.0f, 0.0f);
		...
		while (!glfwWindowShouldClose(window)) {////////////////////////////////////////////////////////////////////////////
			...
			obj6.get<Transform>().RotateLocal(0.0f, 50.0f * time.DeltaTime, 0.0f); 
			obj5.get<Transform>().RotateLocal(0.0f, -50.0f * time.DeltaTime, 0.0f);
